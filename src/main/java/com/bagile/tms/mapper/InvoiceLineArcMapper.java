 ...
 * @author aelguezzar
 */
public class InvoiceLineArcMapper {
    private InvoiceLineArcMapper() {
    }
    ...
    public static InvoiceLineArc toDto(InvInvoiceLineArc invInvoiceLine, boolean lazy) {
        if (null == invInvoiceLine) {
            return null;
        }
        InvoiceLineArc invoiceLine = new InvoiceLineArc();
        ...
        if (!lazy) {
            invoiceLine.setInvoice(InvoiceArcMapper.toDto(invInvoiceLine.getInvInvoice(), true));
    ...
    public static InvInvoiceLineArc toEntity(InvoiceLineArc invoiceLine, boolean lazy) {
        if (null == invoiceLine) {
            return null;
        }
        InvInvoiceLineArc invInvoiceLine = new InvInvoiceLineArc();
        invInvoiceLine.setInvInvoice(InvoiceArcMapper.toEntity(invoiceLine.getInvoice(), true));
    ...
    public static List<InvoiceLineArc> toDtos(List<InvInvoiceLineArc> invInvoiceLines, boolean lazy) {
        if (null == invInvoiceLines) {
            return null;
        }
        List<InvoiceLineArc> invoiceLines = new ArrayList<>();
        for (InvInvoiceLineArc invInvoiceLine : invInvoiceLines) {
            invoiceLines.add(toDto(invInvoiceLine, lazy));
        }
        return invoiceLines;
    }

    public static List<InvoiceLineArc> toDtos(Iterable<InvInvoiceLineArc> invInvoiceLines, boolean lazy) {
        if (null == invInvoiceLines) {
            return null;
        }
        List<InvoiceLineArc> invoiceLines = new ArrayList<>();
    ...
    public static Set<InvInvoiceLineArc> toEntities(Set<InvoiceLineArc> invoiceLines, boolean lazy) {
        if (null == invoiceLines) {
            return null;
        }
        Set<InvInvoiceLineArc> invInvoiceLines = new HashSet<>();
        for (InvoiceLineArc invoiceLine : invoiceLines) {
            invInvoiceLines.add(toEntity(invoiceLine, lazy));
        }
        return invInvoiceLines;
    }

    public static Set<InvoiceLineArc> toDtos(Set<InvInvoiceLineArc> invInvoiceLines, boolean lazy) {
        if (null == invInvoiceLines) {
            return null;
        }
        Set<InvoiceLineArc> invoiceLines = new HashSet<>();
        for (InvInvoiceLineArc invInvoiceLine : invInvoiceLines) {
            invoiceLines.add(toDto(invInvoiceLine, lazy));
        }
        return invoiceLines;
